...
{
	m_model_transform[3][0] = transform.position.x * polarity;
	m_model_transform[3][1] = transform.position.y * polarity;

	m_model_transform = m_model_transform * glm::mat4_cast(glm::angleAxis(glm::radians(transform.rotation * polarity), glm::vec3(0, 0, 1)));
...
{
	m_model_transform = glm::rotate(m_model_transform, glm::radians(degrees * polarity), glm::vec3(0, 0, 1));
}
